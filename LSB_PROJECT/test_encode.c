...
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
   //for reading
	...
	printf("INFO:secret file is present\n");
	//return e_success;
    }
    ...
    if (argv[4] == NULL)
    {
	 encInfo->stego_image_fname = "stego_image.bmp";
         printf("INFO:Output file is not present.created stego_image.bmp as output by default.\n");
    }
    else
    {
...
Status copy_bmp_header(FILE *fptr_src_image, FILE *fptr_dest_image) 
{
    ...
    if (fread(img_head,sizeof(char),53,fptr_src_image) != BMP_HEADER)
    {
	return e_failure;
    }
    //for checking writing of bmp header
    if (fwrite(img_head,sizeof(char),53,fptr_dest_image) != BMP_HEADER)
    {
	return e_failure;
    }
